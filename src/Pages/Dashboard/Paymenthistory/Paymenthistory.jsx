...
const Paymenthistory = () => {
    ...
    return (
      <div>
        <p className="text-orange-600 text-4xl text-center my-10 italic"> Payment History</p>
                ...
                <Tbody>
                  {history.map((eachPayment) => (
                    <Tr key={eachPayment._id}>
                      <Td className="text-red-900">{eachPayment.transactionId}</Td>
                      <Td className="italic">{eachPayment.date}</Td>
                      <Td className="text-amber-700">
                        {eachPayment.totalPrice}
                      </Td>
                      <Td className="text-zinc-700-600">{eachPayment.quantity}</Td>
                      <Td className="text-green-600">{eachPayment.itemsName.map(item=><div>{item}</div>)}</Td>
                    </Tr>
                  ))}
                </Tbody>
              </Table>
